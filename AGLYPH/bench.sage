...
RESULTS_FILE = "bench_results_aglyph.json"
...
prev_name = __name__
__name__ = None
load("AGLYPH/aglyph.sage", globals())
__name__ = prev_name
...
def bench_sign():
    """
    Benchmark the sign method for a single signer.
    This involves commit, sign, and aggregate(1) to produce a verifiable signature.
    The timing includes retries until a valid signature is produced.
    """
    aglyph = AGLPHY()
    pubkey, privkey = aglyph.keygen()
    
    start = CLOCK()
    while True:
        nonces, commitment = aglyph.commit(MESSAGE)
        H = commitment
        sig_part = aglyph.sign(H, privkey, nonces, MESSAGE)
        
        if sig_part is None:
            continue

        aggregated_signature = aglyph.aggregate([sig_part], [pubkey])
        
        if aggregated_signature is not None:
            break
    end = CLOCK()
    return end - start
...
def bench_verify():
    ...
    while True:
        ...
        signature = aglyph.aggregate([sig_part], [pubkey])
        if signature is not None:
            break
    
    start = CLOCK()
    aglyph.verify(MESSAGE, signature, [pubkey])
    end = CLOCK()
    return end - start
...
if __name__ == "__main__":
    print()
    print(f"--- AGLYPH Benchmarks (running {ITERATIONS} iterations) ---\n")
