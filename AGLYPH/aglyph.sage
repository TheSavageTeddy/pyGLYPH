...
class AGLPHY:
    def __init__(self):
        '''
        Initialize RLWE with default parameters
        We use the same parameters GLYPH uses: https://github.com/quantumsafelattices/glyph
        '''
    ...
    def commit(self, msg):
        y1 = gen_bounded_polynomial(self.r, degree = self.n, poly_ring = self.S)
        y2 = gen_bounded_polynomial(self.r, degree = self.n, poly_ring = self.S)

        return (y1, y2), self.a * y1 + y2

    def sign(self, H, private_key, nonce, m):
        '''
    ...
    def aggregate(self, signatures, public_keys):
        Z1 = 0
        Z2 = 0
    ...
    def verify(self, msg: bytes, sig, public_keys) -> bool:
        '''
...
class Validator:
    def __init__(self):
        self.aglyph = AGLPHY()
        self.pubkey, self.privkey = aglyph.keygen()

    def submitNonceCommit(self, msg: bytes):
        self.m = msg
        self.nonces, commitment =  self.aglyph.commit(msg)
        return commitment
    
    def sign(self, challs):
        H = sum(challs)
        return self.aglyph.sign(H, self.privkey, self.nonces, self.m)
